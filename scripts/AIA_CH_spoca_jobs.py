...
aia_file_pattern = (
	'/data/sdo/aia_quicklook/{wavelength:04}/{date:%Y/%m/%d}/aia_quicklook.{wavelength:04}.{date:%Y%m%d_%H}*.fits'
)
...
def parse_tracking_color_file(tracking_color_file):
	try:
		...
		logging.warning('Could not read tracking color from file "%s": %s', tracking_color_file, error)
		return 0
	else:
...
def create_segmented_map(AIA_images, date):
	...
	job = Job(
		classification_exec,
		*AIA_images,
		config=classification_config_file,
		centersFile=classification_centers_file,
		output=segmented_map,
	)
...
def track_maps(tracked_maps, untracked_maps, newly_tracked_maps):
	...
	job = Job(
		tracking_exec,
		*maps,
		config=tracking_config_file,
		maxDeltaT=(tracking_overlap * classification_run_frequency).total_seconds(),
		newColor=last_color,
	)
	...
	if return_code != 0:
		...
		try:
			...
			logging.error('Could not write tracking color to file "%s": %s', tracking_color_file, error)

	...
	def __str__(self):
		...
		if self.message is not None:
			message += self.message.format(
				returncode=self.returncode, stdout=self.stdout, stderr=self.stderr, job_name=self.job_name, **self.extra
			)
		else:
			if self.job_name:
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'parameters', metavar='PARAM', nargs='*', help='Any additional parameter (must be preceded by "--")'
	)

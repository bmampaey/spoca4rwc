...
class EventDB:
	...
	def post_data(self, data):
		...
		try:
			...
			raise EventDBError(str(error)) from error

		for attempt in range(self.max_retries + 1):
			try:
				response = requests.post(
					self.post_address, data=encoded_data, timeout=self.timeout, headers={'Content-Type': 'application/json'}
				)
			except requests.exceptions.RequestException as error:
				logging.error('POST request failed (%s): %s', attempt, error)
	...
	def get_data(self, params=None):
		...
		for attempt in range(self.max_retries + 1):
			try:
				...
				logging.error('GET request failed (%s): %s', attempt, error)
				last_exception = error
			else:
...
if __name__ == '__main__':
	...
	parser.add_argument(
		...
		'-u',
		nargs=2,
		metavar=('FILEPATH', 'METHODPATH'),
		help='Update an event from a JSON file, using merge method',
	)
	...
	if args.get:
		...
		except EventDBError as error:
			logging.error('Failed to get event: %s', error)
		except Exception as error:
			logging.critical('Failed to get event: %s', error)
		else:
		...
		try:
			...
			logging.critical('Could not parse file %s: %s', file_path, error)
			sys.exit(2)
		except Exception as error:
			logging.critical('Could not read file %s: %s', file_path, error)
			sys.exit(2)

		try:
			result = event_db.create_event(event)
		except EventDBError as error:
			logging.error('Failed to create event: %s', error)
		except Exception as error:
			logging.critical('Failed to create event: %s', error)
		else:
		...
		try:
			...
			logging.critical('Failed to find method "%s": %s', method_path, error)
			sys.exit(2)
		...
		try:
			...
			logging.error('Failed to update event: %s', error)
		except Exception as error:
			logging.critical('Failed to update event: %s', error)
		else:

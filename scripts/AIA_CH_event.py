...
from astropy.units import km, pixel
...
def get_spoca_coronal_hole_detection(map, region, region_stat, chaincode, name=None):
	...
	data = {
		...
		'Location': get_heliographic_coordinate(
			map, region_stat['XCENTER'], region_stat['YCENTER'], region['DATE_OBS'] + 'Z', origin=1
		),
...
def get_CHMap_events(map_path):
	...
	chaincodes = {
		id: (chaincodes_hdu.data['X%07d' % id], chaincodes_hdu.data['Y%07d' % id]) for id in regions_hdu.data['ID']
	}

	# Get region stats by channel then by id
	region_stats = {
		hdus[region_stats_hdu_name].header['CHANNEL']: {
			region_stat['ID']: region_stat for region_stat in hdus[region_stats_hdu_name].data
		}
	...
	for id, region in regions.items():
		...
		events['spoca_coronal_hole_detection'] = get_spoca_coronal_hole_detection(
			map, region, next(iter(region_stats.values()))[id], chaincodes[id], name=spoca_coronal_hole_detection_name
		)
		...
		events['spoca_coronal_hole'] = get_spoca_coronal_hole(
			spoca_coronal_hole_detection_name, region['DATE_OBS'] + 'Z', name=spoca_coronal_hole_name
		)

		events['spoca_coronal_hole_statistics'] = []
		for channel, stats in region_stats.items():
			...
			events['spoca_coronal_hole_statistics'].append(
				get_spoca_coronal_hole_statistics(
					spoca_coronal_hole_detection_name, channel, stats[id], name=spoca_coronal_hole_statistics_name
				)
			)
...
def merge_spoca_coronal_hole(spoca_coronal_hole1, spoca_coronal_hole2):
	...
	if spoca_coronal_hole2:
		...
		spoca_coronal_hole1['data']['BeginTime'] = min(
			spoca_coronal_hole1['data']['BeginTime'], spoca_coronal_hole2['data']['BeginTime']
		)
		spoca_coronal_hole1['data']['EndTime'] = max(
			spoca_coronal_hole1['data']['EndTime'], spoca_coronal_hole2['data']['EndTime']
		)

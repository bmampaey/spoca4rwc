...
def get_heliographic_coordinate_stonyhurst(map, x, y, origin=0):
	...
	return (
		stonyhurst.lon.degree,
		stonyhurst.lat.degree,
	)
...
def get_stats(map, region, region_stat):
	...
	center_lon, center_lat = get_heliographic_coordinate_stonyhurst(
		map, float(region_stat['XCENTER']), float(region_stat['YCENTER']), origin=1
	)
...
def get_CHMap_stats(map_path):
	...
	region_stats = {region_stat['ID']: region_stat for region_stat in hdus[region_stats_hdu_name].data}

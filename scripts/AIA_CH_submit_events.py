...
def submit_CHMap_events(map_path, event_db):
	...
	for events in CH_events:
		...
		try:
			...
			logging.error('Failed to submit event "%s": %s', events['spoca_coronal_hole_detection']['name'], error)
		...
		for event in events['spoca_coronal_hole_statistics']:
			if event['data']['PixelsNumber'] > 0:
				try:
					...
					logging.error('Failed to submit event "%s": %s', event['name'], error)
			else:
				logging.warning('SPOCA_CoronalHoleStatistics event "%s" has PixelsNumber <=0, skipping!', event['name'])
		...
		try:
			...
			logging.error('Failed to submit event "%s": %s', events['spoca_coronal_hole']['name'], error)

	# Create the SPOCA_CoronalHoleRun event
	try:
		event_db.create_event(run_event)
	except EventDBError as error:
		logging.error('Failed to submit event "%s": %s', run_event['name'], error)
